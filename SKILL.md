...
它已经适合在 Codex 中手动或半自动执行 bug 分诊，也可以作为 Codex“已安排”任务的执行基础。当前 runner 支持初始化项目配置、从飞书/MCP/导出的 JSON 导入工单、生成 Feishu MQL、生成 bugflow 工件、执行需求-仓库匹配、初步分诊、生成受控修复计划、记录实现/验证/本地闭环。runner 不会自动改代码或远程状态。
...
- `scripts/bugflow_runner.py`：v2 受控修复 runner，支持 `init-project`、`doctor`、`feishu-mql`、`fetch-json`、`triage`、`daily`、`plan-fix`、`record-implementation`、`record-verification`、`close-local`。
