...
```text
Skill + Workflow + 工件化执行框架 + v2 受控修复 runner
```

它已经适合在 Codex 中手动或半自动执行 bug 分诊，也可以作为 Codex“已安排”任务的执行基础。当前 runner 支持初始化项目配置、从飞书/MCP/导出的 JSON 导入工单、生成 Feishu MQL、生成 bugflow 工件、执行需求-仓库匹配、初步分诊、识别纯前端低风险自动修复候选、生成受控修复计划、记录实现/验证/本地闭环，并在验证通过后为单个 bug 创建本地 git 提交。runner 不会自动改远程状态或 push。
...
如果项目还没有配置，先阅读 `references/project-config.md`，再运行 `scripts/bugflow_runner.py init-project` 生成 `.codex/bugflow/` 配置；必要时再参考 `assets/project-config.template.yaml` 或 `assets/feishu-project-config.template.yaml`。没有明确配置前，不要修改远程工单状态。
...
5. 选择修复候选。
   ...
   - 工件化执行时，修改代码前运行 `scripts/bugflow_runner.py plan-fix --issue <id>` 创建或更新 `fix-plan.md`。
   - 默认只修复 `auto-fix-candidate`；纯前端样式、布局、展示类低风险问题可以自动进入该候选。
   - `hard`、`blocked`、`needs-confirmation`、`not-current-repo`、跨团队问题都不能自动修复，除非用户明确指定；明确指定时用 `--approved` 留下批准痕迹。
6. 开始修复流程。
...
7. 实现修复。
   ...
   - 修改代码后运行 `record-implementation` 记录变更摘要、文件和远程状态动作。
8. 验证。

   - 运行项目配置中的格式化、lint、测试、stylelint、构建或回归脚本。
   - 工件化执行时，运行 `record-verification` 创建或更新 `verification.md`。
   - 可见 UI 问题必须阅读 `references/browser-verification.md` 并做浏览器验证，除非用户明确说不需要。
   - 遵守登录策略，不要求用户在聊天里粘贴密码。
   - 验证通过且 `git_policy.auto_commit_after_fix` 为 true 时，运行 `commit-fix --issue <id> --files <changed-files>` 为当前 bug 创建一次本地提交；默认不 push。
9. 闭环。

   - 只有配置或用户允许时，才评论远程工单。
   - 工件化执行时，运行 `close-local` 创建或更新 `closure.md`。
...
- `scripts/bugflow_runner.py`：v2 受控修复 runner，支持 `init-project`、`doctor`、`feishu-mql`、`fetch-json`、`triage`、`daily`、`plan-fix`、`record-implementation`、`record-verification`、`commit-fix`、`close-local`。
