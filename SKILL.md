...
```text
Skill + Workflow + 工件化执行框架 + v2 初始化/分诊 runner
```

它已经适合在 Codex 中手动或半自动执行 bug 分诊，也可以作为 Codex“已安排”任务的执行基础。当前 runner 支持初始化项目配置、从飞书/MCP/导出的 JSON 导入工单、生成 Feishu MQL、生成 bugflow 工件、执行需求-仓库匹配和初步分诊，但不会自动改代码或远程状态。
...
- `scripts/bugflow_runner.py`：v2 初始化/分诊 runner，支持 `init-project`、`doctor`、`feishu-mql`、`fetch-json`、`triage`、`daily`。
